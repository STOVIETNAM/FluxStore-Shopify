...
import '../services/global/home_screen_config.dart';
...
class AppModel with ChangeNotifier {
  ...
  Future<AppConfig?> loadAppConfig({isSwitched = false}) async {
    var startTime = DateTime.now();
    _langCode = _langCode == '' ? kAdvanceConfig['DefaultLanguage'] : _langCode;

    try {
      await HomeScreenConfig.getHomeScreenConfig();
      Configurations.onBoardingDataa =
          HomeScreenConfig.configFromApi['OnBoarding']['data'];
      if (!isInit || _langCode!.isEmpty) {
        await getPrefConfig();
      }

      if (HomeScreenConfig.configFromApi.isNotEmpty) {
        appConfig = AppConfig.fromJson(HomeScreenConfig.configFromApi);
      } else {
  ...
  final TooltipController homePageTooltipController = TooltipController();
  final TooltipController productDetailToolTipController = TooltipController();
}
